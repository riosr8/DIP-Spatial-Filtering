import cv2
import numpy as np
from app.smoothing import gaussian_yourchoice


def unsharp_mask(image, k, filter_size, threshold=0):
    input_image = cv2.imread(image_name, 0)
    m = len(input_image)
    n = len(input_image[0])
    final_image = np.zeros((m, n), dtype=int)

    # Unsharp mask filter
    # smooth_image = cv2.GaussianBlur(image,(filter_size,filter_size),0)  #testing purposes
    smooth_image = gaussian_yourchoice(input_image, filter_size)
    image_difference = cv2.absdiff(input_image, smooth_image)
    actual_mask = k * image_difference
    if(threshold == 0):
        final_image = cv2.add(input_image, actual_mask)
    else:
        for i in range(m):
            for j in range(n):
                if(image_difference[i][j] > threshold):
                    final_image[i][j] = input_image[i][j] + actual_mask[i][j]
                else:
                    final_image[i][j] = input_image[i][j]

    return final_image
